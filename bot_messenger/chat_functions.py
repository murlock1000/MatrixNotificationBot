...
def is_ready_to_send_message(client: AsyncClient, room_id:str, mxid:str):
    msg_room = None
    if room_id in client.rooms:
        msg_room = client.rooms[room_id]
    else:
        return False
    
    if msg_room.encrypted and mxid in msg_room.users.keys():
        return True
    else:
        return False
